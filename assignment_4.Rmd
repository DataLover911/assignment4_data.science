...
author: |
  | Nikita Beloglazovs 
  | n.beloglazovs@students.uu.nl
  | https://github.com/DataLover911
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
GrowthSW$treat <- ifelse(GrowthSW$revolutions == 0, 
                         "Revolutions equal to 0", "More than 0 revolutions")
...
The table compares two groups based on the variables "growth" and "rgdp60." The first group, which had more than 0 revolutions, has a mean growth rate of 1.68 and a median growth rate of 1.92. The second group, with revolutions equal to 0, has a higher mean growth rate of 2.46 and a higher median growth rate of 2.29. It seems that the group without any revolutions experienced slightly higher average growth rates compared to the group with revolutions.
...
```{r}
# Perform t-test on growth variable based on treat groups
...
As the p-value (0.06871) is larger than the common significance level of 0.05, we do not have strong evidence to reject the null hypothesis. Therefore, we cannot conclude that there is a significant difference in the mean growth between the two groups: with revolution and without revolution. However, further analysis could be done. 
...
RGDP60 represents the 1960 GDP per capita in 1960 U.S. dollars. We must take this into account as well because a smaller absolute GDP per capita is predicted to expand more easily than a larger one.
...
The main driver of economic growth appears to be the variable "education." It consistently shows a significant and positive coefficient across all models, indicating a strong positive influence on economic growth. Also, this variable has the greatest R2 in a model, reaching up to 32%, which means that variables here best describe our model.
...
list(model1, model2, model3, model4) |>
  ...
  kable_styling() |>
  column_spec(column = 2, background = "red", color = "white")
...
list(model1, model2, model3, model4) |> 
modelsummary(stars=T, title = "Regression table",   
